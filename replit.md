...
- Category Multi-Select field type uses public endpoint `/api/public/resource-categories` to pull options from ResourceCategory entity (search/content categories), keeping form choices aligned with Category Management. Form builders can configure which specific categories to show by selecting them in the field configuration (`allowed_category_ids` array); if none selected, all categories are displayed
